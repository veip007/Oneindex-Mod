...
//修改过
	class oneindex{
		...
		// 刷新当前目录内需要缓存的内容的缓存
		static function refresh_file_cache($path)
		{
			if (self::refresh_current_cache($path)=='failed')
			{
				return 'failed';
			}
			global $MOD_NEEDCACHE_EXT,$MOD_NEEDCACHE_NAME;
			$items=cache::get('dir_' . $path);//当前目录内容
			foreach((array)$items as $item)//访问每一个文件
			{
				if (!$item["folder"])
				{
					$fileext=pathinfo($item["name"])['extension'];
					if (in_array($fileext,$MOD_NEEDCACHE_EXT)||in_array($item["name"],$MOD_NEEDCACHE_NAME))//判断
					{
						$itempath=self::get_absolute_path($path).$item["name"];
						$resp = fetch::get($item['downloadUrl']);
						cache::set('content_'.$itempath,$resp->content,config('cache_expire_time'));
					}
				}
			}
			return 'success';
